...
export default function Products() {
    const directoryUnitItems = api.directoryUnit.show.useQuery();
    
    return <>
        <Breadcrumbs className="mb-6">
            <BreadcrumbItem><Link shallow={true} href="/dashboard">Главная</Link></BreadcrumbItem>
            <BreadcrumbItem>Единицы измерения</BreadcrumbItem>
        </Breadcrumbs>
        <div className="bg-white max-w-full mx-auto p-6 rounded-xl shadow-sm relative">
            <div className="flex justify-between">
                <h1 className="text-xl font-semibold text-slate-600 mb-8">Единицы измерения</h1>
                <div>
                    <Link shallow href="/dashboard/directory/units/edit">
            ...
            <div>
                <Table aria-label="Example static collection table">
                    ...
                    <TableBody>
                        {
                            directoryUnitItems?.data?.map((item, key) => <TableRow key={key}>
                                <TableCell>{item.title}</TableCell>
                                <TableCell>
                                    <div className="flex gap-2">
                                        <Link shallow={true} href={{
                                            pathname: '/dashboard/directory/units/edit',
                                            query: { id: item.id },
                                        }}>
